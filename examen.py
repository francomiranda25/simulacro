...
class App(customtkinter.CTk):
    ...
    def btn_agregar_on_click(self):
        peso = float(self.txt_peso_articulo.get())
        if peso <= 0:
            alert("peso:", "error")
        else:
            alert("peso:", "correcto")
            if self.combobox_tipo_de_peso.get() == "Gramos":
                self.lista_pesos.append(peso)
            else:
                self.lista_pesos.append(peso * 28.3495)    
    ...
    def btn_mostrar_on_click(self):
        ...
        while i < len(self.lista_pesos):
            print(str(i) + ". Gramos: " + str(self.lista_pesos [i]) + ". Onzas: " + str(self.lista_pesos[i] * 0.035274))
            i = i +1
